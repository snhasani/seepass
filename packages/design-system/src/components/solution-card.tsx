  ...
  ],
  {
    variants: {
      variant: {
        ...
        rejected:
          "border-[--ds-severity-critical]/30 bg-muted/30 opacity-60",
        inProgress:
          "border-[--ds-amber-300] bg-[--ds-problem-bg]",
      },
      size: {
        sm: "p-3 gap-2",
        default: "p-4 gap-3",
...
const statusConfig = {
  proposed: { label: "Proposed", icon: Sparkles, color: "text-[--ds-violet-500]" },
  approved: { label: "Approved", icon: CheckCircle2, color: "text-[--ds-status-active]" },
  rejected: { label: "Rejected", icon: XCircle, color: "text-[--ds-severity-critical]" },
  inProgress: { label: "In Progress", icon: Clock, color: "text-[--ds-amber-500]" },
  completed: { label: "Completed", icon: CheckCircle2, color: "text-[--ds-status-active]" },
};
        ...
        {showEffort && solution.effort && !compact ? (
          <div className="flex gap-2">
            ...
            {solution.effort.human ? (
              <EffortEstimate effort={solution.effort.human} source="human" size="sm" />
            ) : null}
          </div>
        ) : null}
